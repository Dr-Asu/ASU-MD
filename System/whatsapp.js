...
exports.serialize = (ASU, m, options = {}) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = jidNormalizedUser(
      (m.fromMe && ASU.user?.id) || m.key.participant || m.from || ""
    );
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender == jidNormalizedUser(ASU.user && ASU.user?.id);
      m.quoted.text =
        m.quoted.msg?.text ||
      ...
      m.quoted.delete = () =>
        ASU.sendMessage(m.quoted.from, { delete: vM.key });
      m.quoted.download = (pathFile) =>
        ASU.downloadMediaMessage(m.quoted.msg, pathFile);
    }
  }
  m.download = (pathFile) => ASU.downloadMediaMessage(m.msg, pathFile);
  m.body = m.text =
  ...
  m.reply = (text, chatId = m.from, options = {}) =>
    Buffer.isBuffer(text)
      ? ASU.sendFile(chatId, text, "file", "", m, { ...options })
      : ASU.sendText(chatId, text, m, { ...options });
