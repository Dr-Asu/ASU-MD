    ...
    switch (inputCMD) {
      ...
      case "sticker":
        if (/image/.test(mime)) {
          ...
          ASU.sendMessage(m.from, { sticker: stickerBuffer }, { quoted: m });
          ...
          ASU.sendMessage(m.from, { sticker: stickerBuffer2 }, { quoted: m });
        } else {
          await doReact("❌");
          m.reply(
      ...
      case "stickermeme":
        if (/image/.test(mime)) {
          ...
          await doReact("📮");
          media = await ASU.downloadAndSaveMediaMessage(quoted);
      ...
      case "quote":
        if (!text && !m.quoted) {
          await doReact("❔");
          return m.reply(
            `Please provide a text (Type or mention a message) !\n\nExample: ${prefix}q ASU MD is OP`
          );
        }

        if (m.quoted) {
          try {
            userPfp = await ASU.profilePictureUrl(m.quoted.sender, "image");
          } catch (e) {
            userPfp = botImage3;
          }
        } else {
          try {
            userPfp = await ASU.profilePictureUrl(m.sender, "image");
          } catch (e) {
            userPfp = botImage3;
          }
        }
