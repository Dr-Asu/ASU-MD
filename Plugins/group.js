  ...
  start: async (
    ...
    switch (inputCMD) {
      ...
      case "setgcname":
        ...
        try {
          ppgc = await ASU.profilePictureUrl(m.from, "image");
        } catch {
          ppgc = botImage1;
        }

        await ASU.groupUpdateSubject(m.from, text)
          .then((res) =>
            ASU.sendMessage(
              m.from,
              {
      ...
      case "del":
        ...
        if (!isBotAdmin) {
          ...
          await doReact("📛");
          await ASU.sendMessage(m.from, { delete: key });
        } else {
          if (!isAdmin) {
            await doReact("❌");
            return m.reply(
      ...
      case "demote":
        ...
        await doReact("📉");
        try {
          await ASU.groupParticipantsUpdate(m.from, [userId], "demote").then(
            (res) =>
              ASU.sendMessage(
                m.from,
                {
      ...
      case "grouplink":
        ...
        await doReact("🧩");
        let link = await ASU.groupInviteCode(m.from);
      ...
      case "gc":
        ...
        if (text === "close") {
          await ASU.groupSettingUpdate(m.from, "announcement").then((res) =>
            m.reply(`Group has been closed!`)
          );
        } else if (text === "open") {
          await ASU.groupSettingUpdate(m.from, "not_announcement").then(
      ...
      case "leave":
        ...
        await doReact("👋");
        await ASU.sendMessage(m.from, {
          image: { url: "https://wallpapercave.com/uwp/uwp4347654.jpeg" },
          caption: `I'm Leaving this group on request... \n\nTake care everyone :)`,
          mentions: participants.map((a) => a.id),
          quoted: m,
        }).then(async () => {
          ASU.groupLeave(m.from).catch((e) => {
            ASU.sendMessage(
              m.from,
      ...
      case "promote":
        ...
        await doReact("💹");
        try {
          await ASU.groupParticipantsUpdate(m.from, [userId], "promote").then(
            (res) =>
              ASU.sendMessage(
                m.from,
                {
      ...
      case "remove":
        ...
        await ASU.groupParticipantsUpdate(m.from, [users], "remove").then(
          (res) =>
            ASU.sendMessage(
              m.from,
              {
      ...
      case "setppgc":
        ...
        let quotedimage = await ASU.downloadAndSaveMediaMessage(quoted);
      ...
      case "setgcdesc":
        ...
        await ASU.groupUpdateDescription(m.from, newGCdesc).then((res) =>
          ASU.sendMessage(
            m.from,
            {
              image: { url: ppgc, mimetype: "image/jpeg" },
      ...
      case "revoke":
        ...
        if (m.from == "120363040838753957@g.us") {
          await doReact("❌");
          return m.reply(
            "Sorry, this command is not allowed in *ASU Support Group* !\n\nYou are not allowed to change support group link !"
          );
        }
        await doReact("💫");

        await ASU.groupRevokeInvite(m.from).then((res) =>
          ASU.sendMessage(
            m.from,
