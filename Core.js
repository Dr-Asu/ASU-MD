...
module.exports = async (ASU, m, commands, chatUpdate) => {
  try {
    ...
    const metadata = m.isGroup ? await ASU.groupMetadata(from) : {};
    ...
    const groupAdmin = m.isGroup
      ...
      : [];
    const botNumber = await ASU.decodeJid(ASU.user.id);
    ...
    global.suppL = "https://chat.whatsapp.com/DMgVx9WvWlYLtaYWH0rnvm";
    ...
    async function doReact(emoji) {
      ...
      await ASU.sendMessage(m.from, reactm);
    }
    ...
    //if (body.startsWith(prefix) && !icmd)  return ASU.sendMessage(m.from, { text: "Baka no such command" });
    ...
    if (isAntilinkOn && m.isGroup && !isAdmin && !isCreator && isBotAdmin) {
      const linkgce = await ASU.groupInviteCode(from);
